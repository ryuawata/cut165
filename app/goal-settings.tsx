...
export default function GoalSettings({session,profile,goal,target,currentWeightLbs,onClose,onSaved}:{
 ...
 return <div className="settingsShade" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
  <section className="settingsPanel" role="dialog" aria-modal="true" aria-labelledby="goal-settings-title">
   ...
   <form onSubmit={save}>
    <label>Target weight <small>{profile.weight_unit}</small><input type="number" min="1" step=".1" value={targetWeight} onChange={event=>setTargetWeight(event.target.value)} required/></label>
    ...
    <label>Daily step target<input type="number" min="1000" max="100000" step="500" value={stepsTarget} onChange={event=>setStepsTarget(event.target.value)} required/></label>
    <p className="settingsNote">Your current target history stays intact. This version begins {effectiveFrom===today?'today':'tomorrow'}.</p>
    {error&&<p className="formError" role="alert">{error}</p>}
    <button className="primaryAction" disabled={busy}>{busy?'Saving…':'Save goal'} <b>→</b></button>
   </form>
  </section>
 </div>
}
