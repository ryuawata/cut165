...
Phase 4.5 keeps new-user onboarding to three steps: About you, Goal, and Lifestyle / starting plan. A primary calorie point target is presented alongside the persisted calorie range; the plan also shows protein, steps, water, and estimated maintenance. Users can adjust step and water targets before completing setup. Exercise frequency is stored as a constrained profile field for future training recommendations and is deliberately excluded from energy estimation. An aggressive target date is moved to the fastest date consistent with the existing safe weekly rate before targets are persisted. A dismissible, profile-owned first-day prompt never inserts example history.

Phase 4.6 adds deterministic beta coaching without an AI or persistence layer for suggestion strings. `profiles.exercise_frequency` maps to a weekly training recommendation of 2 workouts for `none` or `one_to_two`, 3 for `three_to_four`, and 4 for `five_plus`; changing Profile Settings changes the derived recommendation immediately and does not affect energy estimation. The beta workout rotation alternates Full Body A and B from the user's latest completed `workout_sessions` history. Planned, open, and skipped sessions cannot advance it. Weekly completion is counted from user-scoped sessions using Monday-inclusive and following-Monday-exclusive calendar dates in the profile's IANA timezone. Historical `scheduled_date` values are not rewritten or reinterpreted.

Daily coaching copy is derived in the application domain layer from the current effective target, today's daily metrics and nutrition aggregate, and workout facts. Step guidance uses the remaining positive difference or a reached state. Protein guidance uses known protein only when the aggregate reports no unknown-protein entries; otherwise it reports that the total is incomplete rather than claiming a precise remainder. The UI presents these facts as one restrained dashboard section, and PostgreSQL remains the source of truth for every input.
